    ...
    adm_sta_feature_typ status)
{
    ...
    ent_feature->rel_writing_product_head = NULL;
    ent_feature->rel_writing_product_tail = NULL;

    ent_feature->rel_writing_feature_head = NULL;
    ent_feature->rel_writing_feature_tail = NULL;
    ...
    adm_err_rank_typ error_rank)
{
    ...
    if (source_object != ent_feature) {
        ...
        if (ent_feature->rel_writing_product_head != NULL) {
        ...
        if (ent_feature->rel_writing_feature_head != NULL) {
    ...
    adm_err_rank_typ error_rank)
{
    ...
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_feature_writing_product");
    #endif
    ...
    rel_feature_writing_product = ent_source_feature->rel_writing_product_head;

    while (
        product_not_found &&
        rel_feature_writing_product != NULL) {

        if (rel_feature_writing_product->ent_product == ent_target_product) {

            product_not_found = FALSE;
        }

        rel_feature_writing_product = rel_feature_writing_product->next;
    }

    if (product_not_found) {

        rel_feature_writing_product = (adm_rel_feature_writing_product_typ*)
            adm_allocate_memory(sizeof(adm_rel_feature_writing_product_typ));

        rel_feature_writing_product->ent_product = ent_target_product;

        if (ent_source_feature->rel_writing_product_head == NULL) {

            rel_feature_writing_product->back = NULL;

            ent_source_feature->rel_writing_product_head = rel_feature_writing_product;

        } else {

            rel_feature_writing_product->back = ent_source_feature->rel_writing_product_tail;

            (ent_source_feature->rel_writing_product_tail)->next = rel_feature_writing_product;
        }

        rel_feature_writing_product->next = NULL;

        ent_source_feature->rel_writing_product_tail = rel_feature_writing_product;
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_feature_writing_product");
    #endif
}
    ...
    adm_err_rank_typ error_rank)
{
    ...
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_feature_writing_feature");
    #endif
    ...
    rel_feature_writing_feature = ent_source_feature->rel_writing_feature_head;

    while (
        feature_not_found &&
        rel_feature_writing_feature != NULL) {

        if (rel_feature_writing_feature->ent_feature == ent_target_feature) {

            feature_not_found = FALSE;
        }

        rel_feature_writing_feature = rel_feature_writing_feature->next;
    }

    if (feature_not_found) {

        rel_feature_writing_feature = (adm_rel_feature_writing_feature_typ*)
            adm_allocate_memory(sizeof(adm_rel_feature_writing_feature_typ));

        rel_feature_writing_feature->ent_feature = ent_target_feature;

        if (ent_source_feature->rel_writing_feature_head == NULL) {

            rel_feature_writing_feature->back = NULL;

            ent_source_feature->rel_writing_feature_head = rel_feature_writing_feature;

        } else {

            rel_feature_writing_feature->back = ent_source_feature->rel_writing_feature_tail;

            (ent_source_feature->rel_writing_feature_tail)->next = rel_feature_writing_feature;
        }

        rel_feature_writing_feature->next = NULL;

        ent_source_feature->rel_writing_feature_tail = rel_feature_writing_feature;
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_feature_writing_feature");
    #endif
}
...
{
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_feature_writing_product");
    #endif
    ...
    if (
        (adm_base_integer_typ)rel_feature_writing_product->next !=
        (adm_base_integer_typ)rel_feature_writing_product->ent_product) {

        if (ent_feature->rel_writing_product_head != NULL) {

            if (ent_feature->rel_writing_product_head == rel_feature_writing_product) {

                ent_feature->rel_writing_product_head = rel_feature_writing_product->next;

            } else {

                (rel_feature_writing_product->back)->next = rel_feature_writing_product->next;
            }

            if (ent_feature->rel_writing_product_tail == rel_feature_writing_product) {

                ent_feature->rel_writing_product_tail = rel_feature_writing_product->back;

            } else {

                (rel_feature_writing_product->next)->back = rel_feature_writing_product->back;
            }

            adm_deallocate_memory(rel_feature_writing_product);
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_feature_writing_product");
    #endif
}
...
{
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_feature_writing_feature");
    #endif
    ...
    if (
        (adm_base_integer_typ)rel_feature_writing_feature->next !=
        (adm_base_integer_typ)rel_feature_writing_feature->ent_feature) {

        if (ent_feature->rel_writing_feature_head != NULL) {

            if (ent_feature->rel_writing_feature_head == rel_feature_writing_feature) {

                ent_feature->rel_writing_feature_head = rel_feature_writing_feature->next;

            } else {

                (rel_feature_writing_feature->back)->next = rel_feature_writing_feature->next;
            }

            if (ent_feature->rel_writing_feature_tail == rel_feature_writing_feature) {

                ent_feature->rel_writing_feature_tail = rel_feature_writing_feature->back;

            } else {

                (rel_feature_writing_feature->next)->back = rel_feature_writing_feature->back;
            }

            adm_deallocate_memory(rel_feature_writing_feature);
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_feature_writing_feature");
    #endif
}
