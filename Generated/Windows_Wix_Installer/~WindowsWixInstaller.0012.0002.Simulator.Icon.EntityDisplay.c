    ...
    void)
{
    ...
    width_icon.status = strlen("Status");
    width_icon.atb_processed = strlen("Processed");
    ...
    while (ent_icon != NULL) {
        ...
        width = sprintf_s(buffer, ADM_SIM_BUFFER_SIZE, "%s", adm_sim_status_icon[ent_icon->status]);
        if (width > width_icon.status) width_icon.status = width;

        if (ent_icon->atb_processed == TRUE || ent_icon->atb_processed == FALSE) {
            width = sprintf_s(buffer, ADM_SIM_BUFFER_SIZE, "%s", adm_boolean[ent_icon->atb_processed]);
            if (width > width_icon.atb_processed) width_icon.atb_processed = width;}
    ...
    printf("%-*s", width_icon.status, "Status");
    printf(ADM_SIM_COLUMN_GAP);
    printf("%-*s", width_icon.atb_processed, "Processed");
    printf(ADM_SIM_COLUMN_GAP);
    ...
    printf(ADM_SIM_COLUMN_GAP);
    adm_sim_display_underline(width_icon.status);
    printf(ADM_SIM_COLUMN_GAP);
    adm_sim_display_underline(width_icon.atb_processed);
    printf(ADM_SIM_COLUMN_GAP);
    ...
    while (ent_icon != NULL) {
        ...
        switch (ent_icon->status) {
            case ADM_STATUS_ICON_GENERATING :
                printf("%-*s", width_icon.status, "Generating");
            break;
            default :
                printf("%-*s", width_icon.status, "ERROR");
            break;
        }

        printf(ADM_SIM_COLUMN_GAP);
        if (ent_icon->atb_processed == TRUE || ent_icon->atb_processed == FALSE)
            printf("%-*s", width_icon.atb_processed, adm_boolean[ent_icon->atb_processed]);
        else
            adm_sim_display_row('*', width_icon.atb_processed);
        printf(ADM_SIM_COLUMN_GAP);
