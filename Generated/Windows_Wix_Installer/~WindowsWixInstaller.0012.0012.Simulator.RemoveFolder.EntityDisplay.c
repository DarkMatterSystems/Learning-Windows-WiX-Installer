    ...
    void)
{
    ...
    width_remove_folder.status = strlen("Status");
    ...
    while (ent_remove_folder != NULL) {
        ...
        width = sprintf_s(buffer, ADM_SIM_BUFFER_SIZE, "%s", adm_sim_status_remove_folder[ent_remove_folder->status]);
        if (width > width_remove_folder.status) width_remove_folder.status = width;
    ...
    printf("%-*s", width_remove_folder.status, "Status");
    printf(ADM_SIM_COLUMN_GAP);
    ...
    printf(ADM_SIM_COLUMN_GAP);
    adm_sim_display_underline(width_remove_folder.status);
    printf(ADM_SIM_COLUMN_GAP);
    ...
    while (ent_remove_folder != NULL) {
        ...
        switch (ent_remove_folder->status) {
            case ADM_STATUS_REMOVE_FOLDER_GENERATING :
                printf("%-*s", width_remove_folder.status, "Generating");
            break;
            default :
                printf("%-*s", width_remove_folder.status, "ERROR");
            break;
        }
