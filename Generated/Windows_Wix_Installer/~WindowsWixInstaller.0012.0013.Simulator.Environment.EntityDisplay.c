    ...
    void)
{
    ...
    width_environment.status = strlen("Status");
    ...
    while (ent_environment != NULL) {
        ...
        width = sprintf_s(buffer, ADM_SIM_BUFFER_SIZE, "%s", adm_sim_status_environment[ent_environment->status]);
        if (width > width_environment.status) width_environment.status = width;
    ...
    printf("%-*s", width_environment.status, "Status");
    printf(ADM_SIM_COLUMN_GAP);
    ...
    printf(ADM_SIM_COLUMN_GAP);
    adm_sim_display_underline(width_environment.status);
    printf(ADM_SIM_COLUMN_GAP);
    ...
    while (ent_environment != NULL) {
        ...
        switch (ent_environment->status) {
            case ADM_STATUS_ENVIRONMENT_GENERATING :
                printf("%-*s", width_environment.status, "Generating");
            break;
            default :
                printf("%-*s", width_environment.status, "ERROR");
            break;
        }
