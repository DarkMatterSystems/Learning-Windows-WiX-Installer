    ...
    adm_sta_component_typ status)
{
    ...
    ent_component->rel_writing_directory_head = NULL;
    ent_component->rel_writing_directory_tail = NULL;

    ent_component->rel_writing_feature_head = NULL;
    ent_component->rel_writing_feature_tail = NULL;
    ...
    adm_err_rank_typ error_rank)
{
    ...
    if (source_object != ent_component) {
        ...
        if (ent_component->rel_writing_directory_head != NULL) {
        ...
        if (ent_component->rel_writing_feature_head != NULL) {
    ...
    adm_err_rank_typ error_rank)
{
    ...
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_component_writing_directory");
    #endif
    ...
    rel_component_writing_directory = ent_source_component->rel_writing_directory_head;

    while (
        directory_not_found &&
        rel_component_writing_directory != NULL) {

        if (rel_component_writing_directory->ent_directory == ent_target_directory) {

            directory_not_found = FALSE;
        }

        rel_component_writing_directory = rel_component_writing_directory->next;
    }

    if (directory_not_found) {

        rel_component_writing_directory = (adm_rel_component_writing_directory_typ*)
            adm_allocate_memory(sizeof(adm_rel_component_writing_directory_typ));

        rel_component_writing_directory->ent_directory = ent_target_directory;

        if (ent_source_component->rel_writing_directory_head == NULL) {

            rel_component_writing_directory->back = NULL;

            ent_source_component->rel_writing_directory_head = rel_component_writing_directory;

        } else {

            rel_component_writing_directory->back = ent_source_component->rel_writing_directory_tail;

            (ent_source_component->rel_writing_directory_tail)->next = rel_component_writing_directory;
        }

        rel_component_writing_directory->next = NULL;

        ent_source_component->rel_writing_directory_tail = rel_component_writing_directory;
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_component_writing_directory");
    #endif
}
    ...
    adm_err_rank_typ error_rank)
{
    ...
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_component_writing_feature");
    #endif
    ...
    rel_component_writing_feature = ent_source_component->rel_writing_feature_head;

    while (
        feature_not_found &&
        rel_component_writing_feature != NULL) {

        if (rel_component_writing_feature->ent_feature == ent_target_feature) {

            feature_not_found = FALSE;
        }

        rel_component_writing_feature = rel_component_writing_feature->next;
    }

    if (feature_not_found) {

        rel_component_writing_feature = (adm_rel_component_writing_feature_typ*)
            adm_allocate_memory(sizeof(adm_rel_component_writing_feature_typ));

        rel_component_writing_feature->ent_feature = ent_target_feature;

        if (ent_source_component->rel_writing_feature_head == NULL) {

            rel_component_writing_feature->back = NULL;

            ent_source_component->rel_writing_feature_head = rel_component_writing_feature;

        } else {

            rel_component_writing_feature->back = ent_source_component->rel_writing_feature_tail;

            (ent_source_component->rel_writing_feature_tail)->next = rel_component_writing_feature;
        }

        rel_component_writing_feature->next = NULL;

        ent_source_component->rel_writing_feature_tail = rel_component_writing_feature;
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_component_writing_feature");
    #endif
}
...
{
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_component_writing_directory");
    #endif
    ...
    if (
        (adm_base_integer_typ)rel_component_writing_directory->next !=
        (adm_base_integer_typ)rel_component_writing_directory->ent_directory) {

        if (ent_component->rel_writing_directory_head != NULL) {

            if (ent_component->rel_writing_directory_head == rel_component_writing_directory) {

                ent_component->rel_writing_directory_head = rel_component_writing_directory->next;

            } else {

                (rel_component_writing_directory->back)->next = rel_component_writing_directory->next;
            }

            if (ent_component->rel_writing_directory_tail == rel_component_writing_directory) {

                ent_component->rel_writing_directory_tail = rel_component_writing_directory->back;

            } else {

                (rel_component_writing_directory->next)->back = rel_component_writing_directory->back;
            }

            adm_deallocate_memory(rel_component_writing_directory);
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_component_writing_directory");
    #endif
}
...
{
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_component_writing_feature");
    #endif
    ...
    if (
        (adm_base_integer_typ)rel_component_writing_feature->next !=
        (adm_base_integer_typ)rel_component_writing_feature->ent_feature) {

        if (ent_component->rel_writing_feature_head != NULL) {

            if (ent_component->rel_writing_feature_head == rel_component_writing_feature) {

                ent_component->rel_writing_feature_head = rel_component_writing_feature->next;

            } else {

                (rel_component_writing_feature->back)->next = rel_component_writing_feature->next;
            }

            if (ent_component->rel_writing_feature_tail == rel_component_writing_feature) {

                ent_component->rel_writing_feature_tail = rel_component_writing_feature->back;

            } else {

                (rel_component_writing_feature->next)->back = rel_component_writing_feature->back;
            }

            adm_deallocate_memory(rel_component_writing_feature);
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_component_writing_feature");
    #endif
}
