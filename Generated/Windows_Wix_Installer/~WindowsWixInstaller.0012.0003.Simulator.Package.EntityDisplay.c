    ...
    void)
{
    ...
    width_package.status = strlen("Status");
    ...
    while (ent_package != NULL) {
        ...
        width = sprintf_s(buffer, ADM_SIM_BUFFER_SIZE, "%s", adm_sim_status_package[ent_package->status]);
        if (width > width_package.status) width_package.status = width;
    ...
    printf("%-*s", width_package.status, "Status");
    printf(ADM_SIM_COLUMN_GAP);
    ...
    printf(ADM_SIM_COLUMN_GAP);
    adm_sim_display_underline(width_package.status);
    printf(ADM_SIM_COLUMN_GAP);
    ...
    while (ent_package != NULL) {
        ...
        switch (ent_package->status) {
            case ADM_STATUS_PACKAGE_GENERATING :
                printf("%-*s", width_package.status, "Generating");
            break;
            default :
                printf("%-*s", width_package.status, "ERROR");
            break;
        }
