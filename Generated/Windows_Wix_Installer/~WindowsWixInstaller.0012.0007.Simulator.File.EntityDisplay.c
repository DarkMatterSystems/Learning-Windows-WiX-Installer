    ...
    void)
{
    ...
    width_file.status = strlen("Status");
    ...
    while (ent_file != NULL) {
        ...
        width = sprintf_s(buffer, ADM_SIM_BUFFER_SIZE, "%s", adm_sim_status_file[ent_file->status]);
        if (width > width_file.status) width_file.status = width;
    ...
    printf("%-*s", width_file.status, "Status");
    printf(ADM_SIM_COLUMN_GAP);
    ...
    printf(ADM_SIM_COLUMN_GAP);
    adm_sim_display_underline(width_file.status);
    printf(ADM_SIM_COLUMN_GAP);
    ...
    while (ent_file != NULL) {
        ...
        switch (ent_file->status) {
            case ADM_STATUS_FILE_CREATING :
                printf("%-*s", width_file.status, "Creating");
            break;
            case ADM_STATUS_FILE_GENERATING :
                printf("%-*s", width_file.status, "Generating");
            break;
            case ADM_STATUS_FILE_GENERATING_FILE_SHORTCUT :
                printf("%-*s", width_file.status, "Generating_File_Shortcut");
            break;
            case ADM_STATUS_FILE_IDLE :
                printf("%-*s", width_file.status, "Idle");
            break;
            default :
                printf("%-*s", width_file.status, "ERROR");
            break;
        }
