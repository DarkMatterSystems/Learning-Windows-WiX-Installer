    ...
    adm_err_rank_typ error_rank)
{
    ...
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_EVENT_SENDER, "adm_send_event_product_done");
    #endif

    if (adm_sim_err_success) {

        adm_sim_display_event_product_done(
            ent_product,
            error_file,
            error_line,
            error_rank);
        ...
        pool_target->evente = ADM_EVENT_PRODUCT_DONE;
        pool_target->ent_product = ent_product;
        pool_target->error_location.file = error_file;
        pool_target->error_location.line = error_line;
        pool_target->error_location.rank = error_rank;
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_EVENT_SENDER, "adm_send_event_product_done");
    #endif
}
    ...
    adm_err_rank_typ error_rank)
{
    ...
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_EVENT_SENDER, "adm_send_event_product_complete");
    #endif

    if (adm_sim_err_success) {

        adm_sim_display_event_product_complete(
            ent_product,
            error_file,
            error_line,
            error_rank);
        ...
        pool_target->evente = ADM_EVENT_PRODUCT_COMPLETE;
        pool_target->ent_product = ent_product;
        pool_target->error_location.file = error_file;
        pool_target->error_location.line = error_line;
        pool_target->error_location.rank = error_rank;
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_EVENT_SENDER, "adm_send_event_product_complete");
    #endif
}
