    ...
    adm_sta_directory_typ status)
{
    ...
    ent_directory->rel_writing_product_head = NULL;
    ent_directory->rel_writing_product_tail = NULL;

    ent_directory->rel_writing_directory_head = NULL;
    ent_directory->rel_writing_directory_tail = NULL;
    ...
    adm_err_rank_typ error_rank)
{
    ...
    if (source_object != ent_directory) {
        ...
        if (ent_directory->rel_writing_product_head != NULL) {
        ...
        if (ent_directory->rel_writing_directory_head != NULL) {
    ...
    adm_err_rank_typ error_rank)
{
    ...
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_directory_writing_product");
    #endif
    ...
    rel_directory_writing_product = ent_source_directory->rel_writing_product_head;

    while (
        product_not_found &&
        rel_directory_writing_product != NULL) {

        if (rel_directory_writing_product->ent_product == ent_target_product) {

            product_not_found = FALSE;
        }

        rel_directory_writing_product = rel_directory_writing_product->next;
    }

    if (product_not_found) {

        rel_directory_writing_product = (adm_rel_directory_writing_product_typ*)
            adm_allocate_memory(sizeof(adm_rel_directory_writing_product_typ));

        rel_directory_writing_product->ent_product = ent_target_product;

        if (ent_source_directory->rel_writing_product_head == NULL) {

            rel_directory_writing_product->back = NULL;

            ent_source_directory->rel_writing_product_head = rel_directory_writing_product;

        } else {

            rel_directory_writing_product->back = ent_source_directory->rel_writing_product_tail;

            (ent_source_directory->rel_writing_product_tail)->next = rel_directory_writing_product;
        }

        rel_directory_writing_product->next = NULL;

        ent_source_directory->rel_writing_product_tail = rel_directory_writing_product;
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_directory_writing_product");
    #endif
}
    ...
    adm_err_rank_typ error_rank)
{
    ...
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_directory_writing_directory");
    #endif
    ...
    rel_directory_writing_directory = ent_source_directory->rel_writing_directory_head;

    while (
        directory_not_found &&
        rel_directory_writing_directory != NULL) {

        if (rel_directory_writing_directory->ent_directory == ent_target_directory) {

            directory_not_found = FALSE;
        }

        rel_directory_writing_directory = rel_directory_writing_directory->next;
    }

    if (directory_not_found) {

        rel_directory_writing_directory = (adm_rel_directory_writing_directory_typ*)
            adm_allocate_memory(sizeof(adm_rel_directory_writing_directory_typ));

        rel_directory_writing_directory->ent_directory = ent_target_directory;

        if (ent_source_directory->rel_writing_directory_head == NULL) {

            rel_directory_writing_directory->back = NULL;

            ent_source_directory->rel_writing_directory_head = rel_directory_writing_directory;

        } else {

            rel_directory_writing_directory->back = ent_source_directory->rel_writing_directory_tail;

            (ent_source_directory->rel_writing_directory_tail)->next = rel_directory_writing_directory;
        }

        rel_directory_writing_directory->next = NULL;

        ent_source_directory->rel_writing_directory_tail = rel_directory_writing_directory;
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_link_relation_directory_writing_directory");
    #endif
}
...
{
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_directory_writing_product");
    #endif
    ...
    if (
        (adm_base_integer_typ)rel_directory_writing_product->next !=
        (adm_base_integer_typ)rel_directory_writing_product->ent_product) {

        if (ent_directory->rel_writing_product_head != NULL) {

            if (ent_directory->rel_writing_product_head == rel_directory_writing_product) {

                ent_directory->rel_writing_product_head = rel_directory_writing_product->next;

            } else {

                (rel_directory_writing_product->back)->next = rel_directory_writing_product->next;
            }

            if (ent_directory->rel_writing_product_tail == rel_directory_writing_product) {

                ent_directory->rel_writing_product_tail = rel_directory_writing_product->back;

            } else {

                (rel_directory_writing_product->next)->back = rel_directory_writing_product->back;
            }

            adm_deallocate_memory(rel_directory_writing_product);
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_directory_writing_product");
    #endif
}
...
{
    #if ADM_PROBE
    adm_probe_enter(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_directory_writing_directory");
    #endif
    ...
    if (
        (adm_base_integer_typ)rel_directory_writing_directory->next !=
        (adm_base_integer_typ)rel_directory_writing_directory->ent_directory) {

        if (ent_directory->rel_writing_directory_head != NULL) {

            if (ent_directory->rel_writing_directory_head == rel_directory_writing_directory) {

                ent_directory->rel_writing_directory_head = rel_directory_writing_directory->next;

            } else {

                (rel_directory_writing_directory->back)->next = rel_directory_writing_directory->next;
            }

            if (ent_directory->rel_writing_directory_tail == rel_directory_writing_directory) {

                ent_directory->rel_writing_directory_tail = rel_directory_writing_directory->back;

            } else {

                (rel_directory_writing_directory->next)->back = rel_directory_writing_directory->back;
            }

            adm_deallocate_memory(rel_directory_writing_directory);
    ...
    #if ADM_PROBE
    adm_probe_leave(ADM_PROBE_GROUP_DATA_ACCESS, "adm_kill_relation_directory_writing_directory");
    #endif
}
