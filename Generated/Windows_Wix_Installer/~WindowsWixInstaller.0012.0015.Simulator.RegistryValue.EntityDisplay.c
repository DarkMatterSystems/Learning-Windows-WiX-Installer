    ...
    void)
{
    ...
    width_registry_value.status = strlen("Status");
    ...
    while (ent_registry_value != NULL) {
        ...
        width = sprintf_s(buffer, ADM_SIM_BUFFER_SIZE, "%s", adm_sim_status_registry_value[ent_registry_value->status]);
        if (width > width_registry_value.status) width_registry_value.status = width;
    ...
    printf("%-*s", width_registry_value.status, "Status");
    printf(ADM_SIM_COLUMN_GAP);
    ...
    printf(ADM_SIM_COLUMN_GAP);
    adm_sim_display_underline(width_registry_value.status);
    printf(ADM_SIM_COLUMN_GAP);
    ...
    while (ent_registry_value != NULL) {
        ...
        switch (ent_registry_value->status) {
            case ADM_STATUS_REGISTRY_VALUE_GENERATING :
                printf("%-*s", width_registry_value.status, "Generating");
            break;
            default :
                printf("%-*s", width_registry_value.status, "ERROR");
            break;
        }
