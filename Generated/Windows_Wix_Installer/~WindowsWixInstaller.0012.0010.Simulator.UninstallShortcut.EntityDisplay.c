    ...
    void)
{
    ...
    width_uninstall_shortcut.status = strlen("Status");
    ...
    while (ent_uninstall_shortcut != NULL) {
        ...
        width = sprintf_s(buffer, ADM_SIM_BUFFER_SIZE, "%s", adm_sim_status_uninstall_shortcut[ent_uninstall_shortcut->status]);
        if (width > width_uninstall_shortcut.status) width_uninstall_shortcut.status = width;
    ...
    printf("%-*s", width_uninstall_shortcut.status, "Status");
    printf(ADM_SIM_COLUMN_GAP);
    ...
    printf(ADM_SIM_COLUMN_GAP);
    adm_sim_display_underline(width_uninstall_shortcut.status);
    printf(ADM_SIM_COLUMN_GAP);
    ...
    while (ent_uninstall_shortcut != NULL) {
        ...
        switch (ent_uninstall_shortcut->status) {
            case ADM_STATUS_UNINSTALL_SHORTCUT_GENERATING :
                printf("%-*s", width_uninstall_shortcut.status, "Generating");
            break;
            default :
                printf("%-*s", width_uninstall_shortcut.status, "ERROR");
            break;
        }
